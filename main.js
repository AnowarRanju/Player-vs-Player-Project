...
(function () {
  ...
  const resetBtnElm = document.querySelector('#resetBtn');
    ...
    let p1Score = 0;
    let p2Score = 0;
    let winningScore = Math.floor(Math.random() * 10) + 1; // 1 থেকে 10 এর মধ্যে র‍্যান্ডম সংখ্যা
    let gameOver = false;
    let winner = null;
    let p1Turn = true;
    let p2Turn = false;
    ...
    function resetInput() {
      p1Score = 0;
      p2Score = 0;
      winningScore = Math.floor(Math.random() * 10) + 1; // 1 থেকে 10 এর মধ্যে র‍্যান্ডম সংখ্যা
      gameOver = false;
      winner = null;
